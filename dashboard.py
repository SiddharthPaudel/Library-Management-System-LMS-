...
connector = sqlite3.connect('signup.db')
...
def remove_record():
    ...
    print("DELETE from Library WHERE BK_ID='" + str(selection[1]) + "'")
    print(type(selection[1]))
    # cursor.execute('DELETE from Library WHERE BK_ID=?',str(selection[1]))
    cursor.execute("DELETE from Library WHERE BK_ID='" + str(selection[1]) + "'")
    connector.commit()

    # tree.delete(current_item)
    mb.showinfo('Done', 'The record you wanted was deleted successfully.')
    clear_and_display()
...
root = Tk()
root.title('Softwarica Library Management System')
